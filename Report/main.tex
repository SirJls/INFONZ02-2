...
\renewcommand{\maketitlehookd}{%
  \begin{abstract}
    ...
    deze kennis worden gevolgd; terminologie kan worden gevonden in
    bijlage~\ref{app:Terminologie}
  \end{abstract}
}
...
\begin{appendices}
  ...
  \AppendixItem{Gedistribueerd proces}{
    Een gedistribueerd is een proces waar een enkele taak wordt opgesplitst in
    meerdere subtaken die onafhankelijk berekend worden en vervolgens weer
    worden samengevoegd tot \'e\'en resultaat, zoals te zien in
    figuur~\ref{fig:./fig/distributed_process.png}.
  ...
  \AppendixItem{Multi-core processor}{
    ...
    geheugen, zie figuur~\ref{fig:./fig/mutlicore.png}. Deze processoren zijn
    sneller en hebben een lager energieverbruik dan de single-core
    processor, zie figuur~\ref{fig:./fig/singlecore.png}, echter is het
    programmeren voor deze architectuur een stuk lastiger.
  ...
  \AppendixItem{Parallel proces}{
    Een parallel proces is een vorm van computatie waarin meerdere calculaties
    simultaan kunnen worden uitgevoerd. Deze vorm van executie is sinds de
    komst van de multi-core processor, zie
    figuur~\ref{fig:./fig/mutlicore.png} het dominante paradigma in computer
    architectuur.

    Parallelle processen kunnen in verschillende situatie voorkomen, zoals te
    zien in
    figuur~\ref{fig:./fig/parallel_process_threads.png_./fig/parallel_process_cores.png}.
    In figuur~\ref{fig:./fig/parallel_process_threads.png}
    is er ingezoomd op de parallelle executie van een proces draaiend op
    \'e\'en kern. Een parallelle executie van zo'n proces wordt gedaan door
    gebruik te maken van threads, zie figuur~\ref{fig:./fig/thread.png}. In
    figuur~\ref{fig:./fig/parallel_process_cores.png} zien we een parallel
    executie door middel van een multi-core processor
    (figuur~\ref{fig:./fig/mutlicore.png}, waarbij er gebruik wordt gemaakt van
    de extra kernen beschikbaar voor deze architectuur. Als deze verschillende
    kernen of threads zinnig werk tegelijkertijd doen kan dit het gehele proces
    versnellen.
    \doublefig{0.3}{./fig/parallel_process_threads.png}{Threaded}
    {0.3}{./fig/parallel_process_cores.png}{Kernen}{Parallele processen}
  }

  \newpage

  \AppendixItem{Process \& Thread}{
    Een thread is de kleinste sequentie van een programma dat onafhankelijk kan
    opereren. Hoe een proces en thread opereren hangt af van de implementatie
    van het operatie systeem. De meeste voorkomende implementatie is dat een
    thread een onderdeel is van het proces, zoals te zien in
    figuur~\ref{fig:./fig/thread.png}.
  ...
  \AppendixItem{Single-core Processor}{
    Single-core processoren hebben maar 1 processor direct verbonden aan het
    gerheugen. Deze processoren zijn gemakkelijk te programmeren, maar hebben
    echter een lagere hoog energieverbruik en een lage prestatie, in
    tegenstelling tot de multi-core processor, zie
    figuur~\ref{fig:./fig/mutlicore.png}.
